...
namespace Piwik.Tracker.Tests
{
    ...
    internal class PiwikTrackerTests
    {
        ...
        [TestCase(4, "myVisitVar", "myVisitVarValue")]
        public void GetCustomVariable_WhenScopeIsVisit_ReturnsVariableFromCookie(int variableId, string variableName, string variableValue)
        {
            // Arrange
            using (var ctx = new FakeHttpContext.FakeHttpContext())
            {
                _sut.SetCustomVariable(variableId, variableName, variableValue, Scopes.Visit);
                _sut.SetFirstPartyCookies();
                _sut.ClearCustomVariables();
                // Act
                var actual = _sut.GetCustomVariable(variableId, Scopes.Visit);
                //Assert
                Assert.That(actual.Name, Is.EqualTo(variableName));
                Assert.That(actual.Value, Is.EqualTo(variableValue));
            }
        ...
        [Test]
        [TestCase("www.mydomain.com", "www.mydomain.com")]
        [TestCase("www.mydomain.com.", "www.mydomain.com")]
        [TestCase("*.mydomain.com", "mydomain.com")]
        public void DomainFixup_Tests(string domain, string expected)
        {
            Assert.That(PiwikTracker.DomainFixup(domain), Does.Match(expected));
        }

        [Test]
        public void GetCookieName_Test()
        {
            //Arrange
            _sut.EnableCookies("www.mydomain.com");
            //Act
            var actual = _sut.GetCookieName("mycookie");
            //Assert
            Assert.That(actual, Does.Match("_pk_mycookie.1.4824"));
        }
